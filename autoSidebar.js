...
function walkSync(currentDirPath, prefixBlank, callback) {
  ...
  fs.readdirSync(currentDirPath).forEach(function (name) {
    ...
    } else if (stat.isDirectory() && ".git" != path.basename(filePath) && ".obsidian" != path.basename(filePath) && '_' != path.basename(filePath).substr(0, 1)) {
...
walkSync(curPath, '', function (filePath, stat) {
  if (".md" == path.extname(filePath).toLowerCase()
    ...
    relativeFilePath = itemText + ".md"
    while (itemText.indexOf('/') > 0) {
      itemText = itemText.substr(itemText.indexOf('/') + 1);
      sidebarTxt += '  ';
    }
    let Arr = itemText.toString().split("\\");
    itemText = Arr[Arr.length - 1]
